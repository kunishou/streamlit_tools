...
if sideradio == 'My Movies':
    ...
    st.markdown("### 🎬 PythonのStreamlitを用いて機械学習アプリケーションを作成してみた")
    st.write('')
...
if sideradio == 'About me':
    ...
    image0 = Image.open(requests.get(url0,stream=True).raw).crop((0,0,920,260))
    ...
    ## 趣味
    ```
    機械学習、Pythonの勉強/ デジタルクリエイト/ 音楽/ 一眼カメラ
    ```
    ## 座右の銘
    ```
    バットを振らなきゃホームランは打てない
